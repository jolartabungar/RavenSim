...
class AndGateTest {
  ...
  void constructor_isListeningToAllInputs()
    ...
    new AndGate(context, inputs, output);
    inputs.forEach(
        port -> {
          try {
  ...
  void constructor_invalidNumberOfInputs() {
    Assertions.assertThrows(
        ...
        () -> new AndGate(context, List.of(), Mockito.mock(Port.class)));
  }
  ...
  void update_requestToUpdateIsInvoked()
      throws InvalidBitWidthException, InvalidNumberOfPortsException,
          IncompatibleBitWidthsException {
    new AndGate(context, List.of(new Port(context), new Port(context)), new Port(context))
  ...
  void run_doNotTransmitIfNoSignalChange()
    ...
    new AndGate(context, List.of(new Port(context), new Port(context)), output);
  ...
  void run_transmitIfSignalChange()
    ...
    new AndGate(context, List.of(input0, input1), output);
