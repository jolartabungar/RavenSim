...
class NandGateTest {
  ...
  void constructor_isListeningToAllInputs()
    ...
    new NandGate(context, inputs, output);
    inputs.forEach(
        port -> {
          try {
  ...
  void constructor_invalidNumberOfInputs() {
    Assertions.assertThrows(
        ...
        () -> new NandGate(context, List.of(), Mockito.mock(Port.class)));
  }
  ...
  void update_requestToUpdateIsInvoked()
      throws InvalidBitWidthException, InvalidNumberOfPortsException,
          IncompatibleBitWidthsException {
    new NandGate(context, List.of(new Port(context), new Port(context)), new Port(context))
  ...
  void run_doNotTransmitIfNoSignalChange()
    ...
    new NandGate(context, List.of(new Port(context), new Port(context)), output);
  ...
  void run_transmitIfSignalChange()
    ...
    new NandGate(context, List.of(input0, input1), output);
