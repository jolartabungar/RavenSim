...
class NorGateTest {
  ...
  void constructor_isListeningToAllInputs()
    ...
    new NorGate(context, inputs, output);
    inputs.forEach(
        port -> {
          try {
  ...
  void constructor_invalidNumberOfInputs() {
    Assertions.assertThrows(
        ...
        () -> new NorGate(context, List.of(), Mockito.mock(Port.class)));
  }
  ...
  void update_requestToUpdateIsInvoked()
      throws InvalidBitWidthException, InvalidNumberOfPortsException,
          IncompatibleBitWidthsException {
    new NorGate(context, List.of(new Port(context), new Port(context)), new Port(context))
  ...
  void run_doNotTransmitIfNoSignalChange()
    ...
    new NorGate(context, List.of(new Port(context), new Port(context)), output);
  ...
  void run_transmitIfSignalChange()
    ...
    new NorGate(context, List.of(input0, input1), output);
