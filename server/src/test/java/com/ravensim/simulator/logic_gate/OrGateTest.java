...
class OrGateTest {
  ...
  void constructor_isListeningToAllInputs()
    ...
    new OrGate(context, inputs, output);
    inputs.forEach(
        port -> {
          try {
  ...
  void constructor_invalidNumberOfInputs() {
    Assertions.assertThrows(
        ...
        () -> new OrGate(context, List.of(), Mockito.mock(Port.class)));
  }
  ...
  void update_requestToUpdateIsInvoked()
      throws InvalidBitWidthException, InvalidNumberOfPortsException,
          IncompatibleBitWidthsException {
    new OrGate(context, List.of(new Port(context), new Port(context)), new Port(context))
  ...
  void run_doNotTransmitIfNoSignalChange()
    ...
    new OrGate(context, List.of(new Port(context), new Port(context)), output);
  ...
  void run_transmitIfSignalChange()
    ...
    new OrGate(context, List.of(input0, input1), output);
