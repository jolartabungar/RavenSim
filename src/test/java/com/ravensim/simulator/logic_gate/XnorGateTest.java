...
class XnorGateTest {
  ...
  void constructor_isListeningToAllInputs()
    ...
    new XnorGate(context, inputs, output);
    inputs.forEach(
        port -> {
          try {
  ...
  void constructor_invalidNumberOfInputs() {
    Assertions.assertThrows(
        ...
        () -> new XnorGate(context, List.of(), Mockito.mock(Port.class)));
  }
  ...
  void update_requestToUpdateIsInvoked()
      throws InvalidBitWidthException, InvalidNumberOfPortsException,
          IncompatibleBitWidthsException {
    new XnorGate(context, List.of(new Port(context), new Port(context)), new Port(context))
  ...
  void run_doNotTransmitIfNoSignalChange()
    ...
    new XnorGate(context, List.of(new Port(context), new Port(context)), output);
  ...
  void run_transmitIfSignalChange()
    ...
    new XnorGate(context, List.of(input0, input1), output);
  ...
  void run_correctOutput()
    ...
    Assertions.assertEquals(
        ...
        ~signals.stream()
            .skip(1)
            .reduce(signals.get(0), (accumulator, next) -> accumulator ^ next));
  }
}
