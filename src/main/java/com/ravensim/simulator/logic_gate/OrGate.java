...
public class OrGate extends VariableInput {
  public OrGate(SimulationEngine simulationEngine, List<Port> inputs, Port output)
  ...
  protected BooleanFunction baseCase(BooleanFunction logicGate, Port previousLink, int inputIndex)
    ...
    return new OrDecorator(logicGate, previousLink, io.getInputs().get(inputIndex), io.getOutput());
  }

  @Override
  protected BooleanFunction defaultCase(BooleanFunction logicGate)
      throws IncompatibleBitWidthsException {
    return new OrDecorator(logicGate, io.getInputs().get(0), io.getInputs().get(1), io.getOutput());
  }

  @Override
  protected BooleanFunction initialStep(BooleanFunction logicGate, Port newLink, int inputIndex)
      throws IncompatibleBitWidthsException {
    return new OrDecorator(
  ...
  protected BooleanFunction intermediateStep(
      ...
      throws IncompatibleBitWidthsException {
    return new OrDecorator(logicGate, previousLink, io.getInputs().get(inputIndex), newLink);
  }
}
