...
public class NandGate extends AndGate {
  ...
  public NandGate(SimulationEngine simulationEngine, List<Port> inputs, Port output)
  ...
  protected BooleanFunction baseCase(BooleanFunction logicGate, Port previousLink, int inputIndex)
    ...
    return new NotDecorator(
        new AndDecorator(logicGate, previousLink, io.getInputs().get(inputIndex), newLink),
        newLink,
        io.getOutput());
  }
  ...
  protected BooleanFunction defaultCase(BooleanFunction logicGate)
    ...
    return new NotDecorator(
        new AndDecorator(logicGate, io.getInputs().get(0), io.getInputs().get(1), newLink),
        newLink,
        io.getOutput());
  }
}
